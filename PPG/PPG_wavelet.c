...
// Hàm đọc dữ liệu từ file CSV với cặp giá trị IR và RED
int read_signal_block_from_csv(FILE *file, double *red_signal, double *ir_signal, int block_size) {
    int count = 0;
    while (count < block_size && fscanf(file, "%lf,%lf", &red_signal[count], &ir_signal[count]) == 2) {
        count++;  // Mỗi lần đọc 1 cặp giá trị IR và RED
    }
...
int find_peaks(const double *signal, int signal_len, int *peaks, int max_peaks, double threshold) {
    ...
    while (i < signal_len - 1) {
        // Kiểm tra xem tín hiệu tại vị trí i có lớn hơn cả hai tín hiệu bên cạnh và vượt qua ngưỡng không
        if (signal[i] > threshold && signal[i] > signal[i - 1] && signal[i] > signal[i + 1]) {
            // Nếu số lượng đỉnh chưa vượt quá giới hạn
            if (peak_count < max_peaks) {
                peaks[peak_count++] = i;
                i += 100;  // Nhảy qua các tín hiệu gần nhau (giảm nhiễu)
            } else {
                break;
            }
        }
...
int main() {
    ...
    // Khai báo các mảng cho tín hiệu IR và RED
    double *red_signal = NULL, *ir_signal = NULL;
    int signal_len = 0, capacity = 10000;
    ir_signal = (double *)malloc(capacity * sizeof(double));
    red_signal = (double *)malloc(capacity * sizeof(double));

    // Đọc dữ liệu tín hiệu IR và RED từ file
    signal_len = read_signal_block_from_csv(input_file, red_signal, ir_signal, capacity);
    fclose(input_file);
    // printf("10 mẫu tín hiệu IR và RED đầu tiên:\n");
    // for (int i = 0; i < 10 && i < signal_len; i++) {
    //     printf("Mẫu %d: IR = %.6lf, RED = %.6lf\n", i + 1, ir_signal[i], red_signal[i]);
    // }
    ...
    wavelet_transform(ir_signal, signal_len, cA, &cA_len, cD, &cD_len); //phân tích level 1
    ...
    for (int i = 0; i < peak_count; i++) {
            printf("Đỉnh %d tại vị trí %d\n", i + 1, peaks[i]);
    }
    // Tính SpO2 từ tín hiệu IR và RED
    double spo2 = calculate_spo2(red_signal, ir_signal, signal_len);
    printf("Mức độ bão hòa oxy trong máu (SpO2): %.2f%%\n", spo2);
